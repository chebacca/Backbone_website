...
const EnhancedTeamPage = React.memo(() => {
  ...
  const teamStats = reactExports.useMemo(() => {
    ...
    try {
      ...
      const activeMembers = realTeamMembers.filter((m) => (m == null ? void 0 : m.status) === "ACTIVE").length;
      const pendingInvites = realTeamMembers.filter((m) => (m == null ? void 0 : m.status) === "PENDING").length;
