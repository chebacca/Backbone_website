...
async function fixRemainingLicenseAssignments() {
  console.log('🔧 FIXING REMAINING LICENSE ASSIGNMENTS');
  console.log('=======================================');
  ...
  try {
    ...
    // Identify members without licenses (including those with undefined emails)
    ...
    for (const doc of teamMembersSnapshot.docs) {
      ...
      if (memberLicenses.length === 0) {
        membersWithoutLicenses.push({
          ...
          email: member.email || 'no-email@placeholder.com', // Use placeholder for undefined emails
          role: member.role,
    ...
    membersWithoutLicenses.forEach((member, index) => {
      console.log(`${index + 1}. ${member.email} (${member.name}) - ${member.role} ${!member.hasValidEmail ? '[INVALID EMAIL]' : ''}`);
    });
    ...
    for (let i = 0; i < membersWithoutLicenses.length; i++) {
      ...
      // Create a safe assignment data object
      const assignmentData = {
        userId: member.id,
        email: member.hasValidEmail ? member.email : `placeholder-${member.id}@enterprisemedia.com`,
        name: member.name || `${member.firstName || ''} ${member.lastName || ''}`.trim() || `User ${member.id}`
      };
      ...
      try {
        ...
        assignments.push({
          member: member.email || `placeholder-${member.id}@enterprisemedia.com`,
          memberId: member.id,
          licenseId: licenseDoc.id,
          licenseName: license.name || `License ${licenseDoc.id}`,
          hasValidEmail: member.hasValidEmail
        });
        
        console.log(`   ✅ Assigned license ${licenseDoc.id} to ${member.email || `placeholder-${member.id}@enterprisemedia.com`} ${!member.hasValidEmail ? '[FIXED EMAIL]' : ''}`);
        
      } catch (error) {
        console.error(`   ❌ Failed to assign license to ${member.email || member.id}:`, error.message);
      }
    }
    ...
    if (assignments.length > 0) {
      ...
      assignments.forEach((assignment, index) => {
        console.log(`${index + 1}. ${assignment.member} (${assignment.memberId}) → ${assignment.licenseName} (${assignment.licenseId}) ${!assignment.hasValidEmail ? '[EMAIL FIXED]' : ''}`);
      });
    }
    ...
    console.error('❌ Error fixing remaining license assignments:', error);
    throw error;
  }
}

// Run the fix
fixRemainingLicenseAssignments()
  .then(result => {
    if (result.success) {
      console.log('\n🎉 Remaining license assignment fix completed successfully!');
